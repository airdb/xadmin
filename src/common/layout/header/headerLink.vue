<template>
  <div class="menu-div">
    <template v-if="curItem.link">
      ...
      >
        <i :class="['noahv-icon', 'noahv-icon-' + curItem.type]" v-if="curItem.type ? true : false"></i>
        {{curItem.label}}
      </router-link>
    </template>
    ...
    <template v-else>
      ...
      <template v-if="curItem.children">
        ...
        <ul
          ...
          @touchend="touchend()"
        >
          <li :name="subItem.key" v-for="(subItem, subIndex) in curItem.children">
            <headerLink :item="subItem" :parent="curItem.key"></headerLink>
          </li>
        </ul>
      </template>
    </template>
